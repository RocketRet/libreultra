 ...
 ***********************************************************************/
Acmd *n_alEnvmixerPull(N_PVoice *filter, s64 sampleOffset, Acmd *p) 
{
  Acmd        *ptr = p;
...
// #endif /* _DEBUG */
    ...
    case (AL_FILTER_START_VOICE_ALT):
      {                  
	N_ALStartParamAlt *param = (N_ALStartParamAlt *)e->em_ctrlList;
    ...
    case (AL_FILTER_START_VOICE):
      {
	N_ALStartParam *p = (N_ALStartParam *)e->em_ctrlList;
  ...
  n_alEnvmixerParam(N_PVoice *filter, s32 paramID, void *param)
{
  ...
  case (AL_FILTER_ADD_UPDATE):
    if (e->em_ctrlTail) {
      e->em_ctrlTail->next = (N_ALParam *)param;
    } else {
      e->em_ctrlList = (N_ALParam *)param;
    }            
    e->em_ctrlTail = (N_ALParam *)param;
    break;
